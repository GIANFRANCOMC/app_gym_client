...
async function finalizarModulo(id, estado = "finalizado") {
    ...
    return new Promise(resolve => {
        ...
        .then((response) => {
            ...
            if(response.data.bool) {
                ...
                let aperturaModuloCursoDetalles = response.data?.aperturaModuloCursoDetalles;

                if(aperturaModuloCursoDetalles) {

                    let htmlSwal = "";

                    for(const [key, value] of Object.entries(aperturaModuloCursoDetalles)) {

                        htmlSwal += `<tr>
                                        <td class="align-middle">${parseInt(key) + 1}</td>
                                        <td class="align-middle">${value.moduloCurso?.curso?.nombre}</td>
                                        <td class="align-middle">${obtenerEstadoLegible(value.estado)}</td>
                                     </tr>`;
                            ...
                            `<table class="table table-head-fixed text-nowrap">
                                <thead>
                                    <tr class="text-center">
                                        <th class="bg-primary align-middle">N°</th>
                                        <th class="bg-primary align-middle">Curso</th>
                                        <th class="bg-primary align-middle">Estado</th>
                                    </tr>
                                </thead>
                    ...
                    errorSwal({descripcion: response.data.msg});
        ...
        .catch((error) => {
            ...
            errorSwal({descripcion: defaultDescripcionResponse(error.response.status)});
            resolve(false);
...
async function finalizarCurso(id, estado = "finalizado") {
